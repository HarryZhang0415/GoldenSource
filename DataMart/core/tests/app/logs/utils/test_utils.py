...
import pytest
from datamart_core.app.logs.utils.utils import get_app_id, get_log_dir, get_session_id
...
def test_get_app_id_success():
    ...
    with patch("datamart_core.app.logs.utils.utils.get_log_dir") as mock_get_log_dir:
        mock_get_log_dir
...
def test_get_log_dir():
    with patch(
        "datamart_core.app.logs.utils.utils.create_log_dir_if_not_exists",
        return_value="/test_dir",
    ) as mock_create_log_dir, patch(
        "datamart_core.app.logs.utils.utils.create_log_uuid_if_not_exists",
        return_value="12345",
    ) as mock_create_log_uuid, patch(
        "datamart_core.app.logs.utils.utils.create_uuid_dir_if_not_exists",
