...
from datamart_core.app.static.package_builder import (
    ClassDefinition,
    DocstringGenerator,
    ImportDefinition,
    MethodDefinition,
...
def tmp_datamart_dir(tmp_path_factory):
    return tmp_path_factory.mktemp("datamart_core")


@pytest.fixture(scope="module")
def package_builder(tmp_datamart_dir):
    """Return package builder."""
    return PackageBuilder(tmp_datamart_dir)
...
def test_build(class_definition):
    """Test build."""
    code = class_definition.build("datamart_core.app.static.container.Container")
    assert code
...
def test_build_class_loader_method(method_definition):
    ...
    code = method_definition.build_class_loader_method(
        "datamart_core.app.static.container.Container"
    )
    assert code
...
@patch("datamart_core.app.static.package_builder.MethodDefinition")
...
def test_build_command_method_body(method_definition):
    ...
    with patch(
        "datamart_core.app.static.package_builder.MethodDefinition.is_data_processing_function",
        return_value=False,
    ), patch(
        "datamart_core.app.static.package_builder.MethodDefinition.is_deprecated_function",
        return_value=False,
    ):
        output = method_definition.build_command_method_body(
            path="datamart_core.app.static.container.Container", func=some_func
        )
...
def test_build_command_method(method_definition):
    ...
    with patch(
        ...
        output = method_definition.build_command_method(
            path="datamart_core.app.static.container.Container",
            func=some_func,
            model_name=None,
        )
...
def test_import_definition_build(import_definition):
    ...
    code = import_definition.build(path="datamart_core.app.static.container.Container")
    assert code
...
def test_read_extension_map(package_builder, tmp_datamart_dir):
    """Test read extension map."""

    PATH = "datamart_core.app.static.package_builder."
    open_mock = mock_open()
    with patch(PATH + "open", open_mock), patch(PATH + "load") as mock_load:
        package_builder._read(Path(tmp_datamart_dir / "assets" / "extension_map.json"))
        open_mock.assert_called_once_with(
            Path(tmp_datamart_dir / "assets" / "extension_map.json")
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "datamart_core_extension": [
                    "ext_1@0.0.0",
                    "ext_2@0.0.0",
                ],
                "datamart_provider_extension": [
                    "prov_1@0.0.0",
                    "prov_2@1.1.1",
                ],
            },
            EntryPoints(
                (
                    EntryPoint(
                        name="ext_2", value="...", group="datamart_core_extension"
                    ),
                    EntryPoint(
                        name="prov_2", value="...", group="datamart_provider_extension"
                    ),
                )
            ),
            "0.0.0",
        ...
        (
            {
                "datamart_core_extension": ["ext_1@9.9.9"],
                "datamart_provider_extension": ["prov_2@0.0.0"],
            },
            EntryPoints(
                (
                    EntryPoint(
                    ...
                    EntryPoint(
                        name="prov_1", value="...", group="datamart_provider_extension"
                    ),
                )
            ),
            "5.5.5",
...
@pytest.mark.parametrize(
    "add, remove, datamart_auto_build",
    [
        (set(), set(), True),
        ({"this"}, set(), True),
        (set(), {"that"}, True),
...
def test_auto_build(package_builder, tmp_datamart_dir, add, remove, datamart_auto_build):
    """Test auto build."""

    with patch.object(PackageBuilder, "_diff") as mock_assets_diff, patch.object(
        PackageBuilder, "build"
    ) as mock_build, patch.object(Env, "AUTO_BUILD", datamart_auto_build):
        mock_assets_diff.return_value = add, remove

        package_builder.auto_build()

    if datamart_auto_build:
