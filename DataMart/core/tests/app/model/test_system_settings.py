...
def test_create_datamart_directory_directory_and_files_not_exist(tmpdir):
    # Arrange
    values = MockSystemSettings(
        **{
            "datamart_directory": str(tmpdir.join("datamart_core")),
    ...
    # Act
    SystemSettings.create_datamart_directory(values)  # type: ignore[operator]

    # Assert
    assert os.path.exists(values.datamart_directory)  # type: ignore[attr-defined]
...
def test_create_datamart_directory_directory_exists_user_settings_missing(tmpdir):
    # Arrange
    values = MockSystemSettings(
        **{
            "datamart_directory": str(tmpdir.join("datamart_core")),
    ...
    # Create the datamart_core directory
    Path(values.datamart_directory).mkdir(parents=True, exist_ok=True)  # type: ignore[attr-defined]
...
def test_create_datamart_directory_directory_exists_system_settings_missing(tmpdir):
    # Arrange
    values = MockSystemSettings(
        **{
            "datamart_directory": str(tmpdir.join("datamart_core")),
