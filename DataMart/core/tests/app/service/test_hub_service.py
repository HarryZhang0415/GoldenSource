...
from datamart_core.app.service.hub_service import (
    Credentials,
    HubService,
    HubSession,
    HubUserSettings,
...
def mocker():
    """Fixture for mocker."""
    with patch("datamart_core.app.service.hub_service.HubService") as mock:
        yield mock
...
def test_get_session_from_email_password():
    ...
    with patch(
        "datamart_core.app.service.hub_service.post",
...
def test_disconnect():
    ...
    with patch(
        "datamart_core.app.service.hub_service.get",
        return_value=MagicMock(
            status_code=200,
...
def test_put_user_settings():
    ...
    with patch(
        "datamart_core.app.service.hub_service.put",
        return_value=MagicMock(
            status_code=200,
        ),
    ):
