...
from datamart_charting.core.backend import PLOTLYJS_PATH, create_backend, get_backend
from datamart_charting.core.chart_style import ChartStyle
from datamart_charting.core.config.datamart_styles import (
    PLT_TBL_ROW_COLORS,
)

if TYPE_CHECKING:
    from datamart_core.app.model.charts.charting_settings import ChartingSettings
...
class DataMartFigure(go.Figure):
    """Custom Figure class for DataMart Terminal.
    ...
    **kwargs
    ...
    Class Methods
    -------------
    create_subplots(rows: `int`, cols: `int`, **kwargs) -> `DataMartFigure`
    ...
    to_subplot(subplot: `DataMartFigure`, row: `int`, col: `int`, secondary_y: `bool`, ...)
    ...
    def to_subplot(
        ...
        **kwargs,
        ...
        subplot : `plotly.graph_objects.Figure`
            Figure object created with `DataMartFigure.create_subplots` / `plotly.subplots.make_subplots`
        row : `int`
            Row number
        col : `int`
            Column number
    ...
    def _add_cmd_source(self, command_location: Optional[str] = "") -> None:
        """Set the watermark for DataMart Terminal."""
        if command_location:
