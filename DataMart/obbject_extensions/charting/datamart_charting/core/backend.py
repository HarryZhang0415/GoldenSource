...
class Backend(PyWry):
    ...
    def __init__(
        ...
        daemon: bool = True,
        max_retries: int = 30,
        proc_name: str = "DataMart Terminal",
    ):
    ...
    def get_kwargs(self, title: Optional[str] = "") -> dict:
        ...
        return {
            "title": "DataMart Platform" + (f" - {title}" if title else ""),
    ...
    def call_hub(self, login: bool = True) -> Optional[dict]:
        ...
        outgoing = dict(
            json_data=dict(url=f"https://my.datamart.co/{endpoint}?pywry=true"),
        ...
        messages_dict = dict(
            login=dict(
                message="Welcome to DataMart Terminal! Please login to continue.",
