...
from datamart_core.app.model.example import APIEx, PythonEx
from datamart_core.app.model.obbject import OBBject
from datamart_core.app.router import Router
from datamart_core.app.utils import basemodel_to_df, get_target_column, get_target_columns
...
from datamart_econometrics.utils import get_engle_granger_two_step_cointegration_test
...
@router.command(
    ...
    examples=[
        PythonEx(
            ...
            code=[
                "stock_data = market.equity.price.historical(symbol='TSLA', start_date='2023-01-01', provider='fmp').to_df()",  # noqa: E501
...
@router.command(
    ...
    examples=[
        PythonEx(
            ...
            code=[
                "stock_data = market.equity.price.historical(symbol='TSLA', start_date='2023-01-01', provider='fmp').to_df()",  # noqa: E501  pylint: disable=line-too-long
                'market.econometrics.ols_regression_summary(data=stock_data, y_column="close", x_columns=["open", "high", "low"])',  # noqa: E501  pylint: disable=line-too-long
            ],
        ),
        APIEx(
            parameters={
...
@router.command(
    ...
    examples=[
        PythonEx(
            ...
            code=[
                "stock_data = market.equity.price.historical(symbol='TSLA', start_date='2023-01-01', provider='fmp').to_df()",  # noqa: E501
                'market.econometrics.residual_autocorrelation(data=stock_data, y_column="close", x_columns=["open", "high", "low"])',  # noqa: E501  pylint: disable=line-too-long
            ],
        ),
        APIEx(
            parameters={
