...
def test_{test_name_extra}{route.replace("/", "_")[1:]}(params, headers):
    ...
    url = f"http://localhost:8001/api/v1{route}?{{query_str}}"
...
def write_commands_integration_tests(
    ...
    for route in cm_map:
        ...
        menu = route.split("/")[1]
        path = os.path.join(
            "datamart", "extensions", menu, "integration", f"test_{menu}_api.py"
        )
...
if __name__ == "__main__":
    ...
    r = requests.get("http://localhost:8001/openapi.json", timeout=10).json()
