...
def test_derivatives_options_chains(params, market):
    result = market.derivatives.options.chains(**params)
    assert result
...
@pytest.mark.integration
def test_derivatives_options_unusual(params, market):
    result = market.derivatives.options.unusual(**params)
    assert result
    assert isinstance(result, OBBject)
    assert len(result.results) > 0
...
@pytest.mark.integration
def test_derivatives_futures_historical(params, market):
    result = market.derivatives.futures.historical(**params)
    assert result
    assert isinstance(result, OBBject)
    assert len(result.results) > 0
...
@pytest.mark.integration
def test_derivatives_futures_curve(params, market):
    result = market.derivatives.futures.curve(**params)
    assert result
    assert isinstance(result, OBBject)
    assert len(result.results) > 0
