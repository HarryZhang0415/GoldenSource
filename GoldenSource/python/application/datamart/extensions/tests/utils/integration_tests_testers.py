...
def get_integration_tests(
    ...
    """Get integration tests for the DataMart Platform."""
                ...
                if provider in command_params:
                    for param in test_params:
                        if (
                            ...
                            and param
                            not in command_params["datamart"]["QueryParams"]["fields"]  # type: ignore
                            and param != "provider"
                        ):
                            wrong_params.append(
                ...
                for param in test_params:
                    ...
                    for provider in providers:
                        if (
                            ...
                            or param
                            in command_params["datamart"]["QueryParams"]["fields"]
                            or param == "provider"
                        ):
    ...
    if not processing:
        for i, test_params in enumerate(function_params):
            if "provider" in test_params and i != 0:
                ...
                for expected_param in command_params["datamart"]["QueryParams"]["fields"]:
                    if expected_param not in test_params:
                        missing_params.append(
