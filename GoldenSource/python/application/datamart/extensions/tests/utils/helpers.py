...
def get_packages_info() -> Dict[str, str]:
    ...
    package_paths = glob.glob("datamart/datamart/package/*.py")
...
def execute_docstring_examples(module_name: str, path: str) -> List[str]:
    ...
    errors = []
    module = importlib.import_module(f"datamart.package.{module_name}")
...
def list_datamart_extensions() -> Tuple[Set[str], Set[str], Set[str]]:
    """List installed datamart extensions and providers.
    ...
    for entry_point in entry_points_dict.get("datamart_core_extension", []):
        core_extensions.add(f"{entry_point.name}")

    for entry_point in entry_points_dict.get("datamart_provider_extension", []):
        provider_extensions.add(f"{entry_point.name}")

    for entry_point in entry_points_dict.get("datamart_obbject_extension", []):
...
def find_decorator(file_path: str, function_name: str) -> str:
    ...
    file_path = os.path.join(
        this_dir.split("datamart/")[0], "datamart", file_path
    )
...
def get_required_fields(model: str) -> List[str]:
    ...
    fields = pi.map[model]["datamart"]["QueryParams"]["fields"]
