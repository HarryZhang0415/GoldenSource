...
def test_{test_name}(params, market):
    params = {{p: v for p, v in params.items() if v}}

    result = market.{command_name}(**params)
    assert result
...
def find_extensions(filter_chart: Optional[bool] = True):
    ...
    extensions = [x for x in Path("datamart/extensions").iterdir() if x.is_dir()]
    extensions.extend(
        [x for x in Path("datamart/obbject_extensions").iterdir() if x.is_dir()]
    )
...
def get_test_params(
    ...
    standard_params = get_model_params(
        param_fields=provider_interface_map[model_name]["datamart"]["QueryParams"][
            "fields"
        ]
    )
...
def write_charting_extension_integration_tests():
    ...
    import datamart_charting  # pylint: disable=import-outside-toplevel

    functions = Charting.functions()

    charting_ext_path = Path(datamart_charting.__file__).parent.parent
