...
def test_derivatives_options_chains(params, headers):
    ...
    url = f"http://localhost:8001/api/v1/derivatives/options/chains?{query_str}"
...
def test_derivatives_options_unusual(params, headers):
    ...
    query_str = get_querystring(params, [])
    url = f"http://localhost:8001/api/v1/derivatives/options/unusual?{query_str}"
    result = requests.get(url, headers=headers, timeout=10)
    assert isinstance(result, requests.Response)
    assert result.status_code == 200
...
def test_derivatives_futures_historical(params, headers):
    ...
    query_str = get_querystring(params, [])
    url = f"http://localhost:8001/api/v1/derivatives/futures/historical?{query_str}"
    result = requests.get(url, headers=headers, timeout=10)
    assert isinstance(result, requests.Response)
    assert result.status_code == 200
...
def test_derivatives_futures_curve(params, headers):
    ...
    query_str = get_querystring(params, [])
    url = f"http://localhost:8001/api/v1/derivatives/futures/curve?{query_str}"
