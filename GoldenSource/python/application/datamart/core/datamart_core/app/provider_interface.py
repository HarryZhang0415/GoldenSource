...
from datamart_core.provider.query_executor import QueryExecutor
from datamart_core.provider.registry_map import MapType, RegistryMap
from datamart_core.provider.utils.helpers import to_snake_case
...
class ProviderInterface(metaclass=SingletonMeta):
    ...
    def _create_field(
        ...
        provider_field = (
            f"(provider: {provider_name})" if provider_name != "datamart" else ""
        )
        description = (
    ...
    def _extract_params(
        ...
        for provider_name, model_details in providers.items():
            if provider_name == "datamart":
                ...
                for name, field in model_details["QueryParams"]["fields"].items():
                    if name not in providers["datamart"]["QueryParams"]["fields"]:
            ...
            if provider_name == "datamart":
                for name, field in model_details["Data"]["fields"].items():
                    if (
                        ...
                        continue
                    incoming = cls._create_field(name, field, "datamart")
                ...
                for name, field in model_details["Data"]["fields"].items():
                    if name not in providers["datamart"]["Data"]["fields"]:
                        if (
                            name == "provider"
                            and field.description == "The data provider for the data."
                        ):  # noqa
    ...
    def _generate_model_providers_dc(self, map_: MapType) -> Dict[str, ProviderChoices]:
        ...
        for model_name, providers in map_.items():
            ...
            if "datamart" in choices:
                choices.remove("datamart")
