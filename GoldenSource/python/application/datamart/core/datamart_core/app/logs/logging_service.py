...
from datamart_core.app.logs.formatters.formatter_with_exceptions import (
...
class LoggingService(metaclass=SingletonMeta):
    ...
    def log(
        ...
        if "login" in route:
            ...
            # Construct message
            message_label = "ERROR" if datamart_error else "CMD"
            log_message = json.dumps(
                {
                    "route": route,
                    "input": kwargs,
                    "error": str(datamart_error.original) if datamart_error else None,
                    "custom_headers": custom_headers,
                },
                default=to_jsonable_python,
            )
            ...
            log_level = logger.error if datamart_error else logger.info
            log_level(
                log_message,
