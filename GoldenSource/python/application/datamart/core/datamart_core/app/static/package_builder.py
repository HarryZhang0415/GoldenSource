...
from datamart_core.app.router import RouterLoader
from datamart_core.app.static.utils.console import Console
from datamart_core.app.static.utils.linters import Linters
from datamart_core.env import Env
from datamart_core.provider.abstract.data import Data

try:
    from datamart_charting import Charting  # type: ignore
    ...
    def build(path: str, ext_map: Optional[Dict[str, List[str]]] = None) -> str:
        ...
        if not path:
            if ext_map:
                ...
                doc += "Extensions:\n"
                doc += "\n".join(
                    [f"    - {ext}" for ext in ext_map.get("datamart_core_extension", [])]
                )
                doc += "\n\n"
                doc += "\n".join(
                    [
                        f"    - {ext}"
                        for ext in ext_map.get("datamart_provider_extension", [])
                    ]
                )
            doc += '    """\n'
            doc += "    # fmt: on\n"
...
class MethodDefinition:
    ...
    def build_func_params(formatted_params: OrderedDict[str, Parameter]) -> str:
        ...
        func_params = func_params.replace(
            "datamart_core.provider.abstract.data.Data", "Data"
        )
...
class DocstringGenerator:
    ...
    def get_field_type(
        ...
        try:
            ...
            if "datamart_" in str(_type):
                _type = (
                    str(_type).split(".", maxsplit=1)[0].split("datamart_")[0]
    ...
    def build_examples(
        ...
        if examples:
            ...
            doc = f"\n{indent}Examples\n"
            doc += f"{indent}--------\n"
            doc += f"{indent}{prompt}from datamart import market\n"
    ...
    def get_provider_field_params(
        ...
        ----------
            ...
            provider (str, optional):
                Provider name. Defaults to "datamart".
            ...
            if route_method == {"GET"}:
                ...
                for provider in model_map:
                    if provider == "datamart":
                        # datamart provider is always present hence its the standard field
                        reference[path]["parameters"]["standard"] = (
                            cls.get_provider_field_params(standard_model, "QueryParams")
                        )
                        # Add `provider` parameter fields to the datamart provider
