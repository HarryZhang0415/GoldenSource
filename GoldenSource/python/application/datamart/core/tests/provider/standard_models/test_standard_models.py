...
import pytest
from datamart_core.provider.abstract.fetcher import Data, QueryParams
...
for model_file in model_files:
    ...
    model_module = import_module(
        f"datamart_core.provider.standard_models.{model_file.stem}"
    )
        ...
        if "QueryParams" in standard_model.__name__:
            if name in QUERY_DESCRIPTIONS:
                assert QUERY_DESCRIPTIONS[name] in getattr(field, "description"), (
                    ...
                    f"To get a full list of reserved descriptions, navigate to datamart_core.provider.utils.descriptions.py"
